 ...
 * $Revision: 10 $
 * $Date: 15/07/06 2:43p $
 * @brief    MINI55 series SPI driver source file
 *
 * @note
...
{
    /* Reset SPI */
    SYS->IPRST1 |= SYS_IPRST1_SPIRST_Msk;
    SYS->IPRST1 &= ~SYS_IPRST1_SPIRST_Msk;
}
...
uint32_t SPI_SetBusClock(SPI_T *spi, uint32_t u32BusClock)
{
    ...
    if((CLK->CLKSEL1 & CLK_CLKSEL1_SPISEL_Msk) == CLK_CLKSEL1_SPISEL_HXTorLXT)
    {
